  ...
  userid=session.getAttribute("s_userid").toString();
	...
	if (errorMsgs.size() == 0){
		try {
			...
			stmt = conn.prepareStatement("UPDATE rooms "+" SET userid=?,name=?,location=?,distance=?,type=?,kind=?,price=?,deposit=?,address=?,lat=?,lng=?,facility=?,description=?,photo=?"+"WHERE roomid=?");
